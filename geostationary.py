...
def mudar_Wallpaper():
    CAMINHO = r"C:\Users\Guterman Junior\Pictures\WallpaperGOES"
    Nome = flistapasta_Filter()[-1]
    WALLPAPER_PATH = CAMINHO + "\\" + Nome
    image = Image.open(WALLPAPER_PATH)

    # Diminuição de resolução para ajuste de tela no Windows 10
    # new_image = image.resize((1080,1080))
    # new_image.save('TEMP\\1080x1080_' + Nome)
    # WALLPAPER_PATH = r'C:\Users\Guterman Junior\Pictures\WallpaperGOES\TEMP\\1080x1080_' + str(Nome)
...
def fremove_old_files():
    arquivos_pasta = flistapasta_Filter()
    deltas = fdeltas(entrada_horas())
    ultimos_arquivos = []
    for delta in deltas:
        nome = fdownload_file(delta)
        ultimos_arquivos.append(nome)
    #imprimir(ultimos_arquivos)
    arquivos_apagar = []
    for arquivo in arquivos_pasta:
        if arquivo not in ultimos_arquivos:
            arquivos_apagar.append(arquivo)
    #imprimir(arquivos_apagar)

    for arquivo_a_deletar in arquivos_apagar:
        if os.path.exists(arquivo_a_deletar):
            os.remove(arquivo_a_deletar)
        else:
            print(f"Não foi possível apagar o {arquivo_a_deletar}")
